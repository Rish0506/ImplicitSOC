...
def main():
    ...
    np.random.seed(seed)
    config_mb_oc = {'batch_size': 1, 'nt': 1, 't_final': 4.0, 'alpha_interaction': 5.0, 'alphaG': 500.0, 'alphaHJB': [1.0e-4, 1.0e-2], 'pen_pos': True, 'ic_var': 0.1}#nt=40 alpha_interaction=25.0 alphaG=100.0
    ...
    for n in np.arange(n_trials):
        run_mb_jfb(config_mb_oc, config_mb_train, N, full_AD_mode=False, device=device, load_prev_model=False, model_path=None)
